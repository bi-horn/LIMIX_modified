...
from limix_modified._bits.array import unvec
...
from limix_modified._display._session import session_line
from ._assert import assert_finite
from .result._ifactory import IScanResultFactory
...
def iscan(G, y, lik="normal", K=None, M=None, idx=None, E0=None, E1=None, verbose=True):
    ...
    with session_block("QTL analysis", disable=not verbose):
        ...
        if idx is None:
            ...
            assert E1.shape[1] > 0 # sets idx to cover all columns in G 
            idx = range(G.shape[1])

            if E0.shape[1] == 0: # no environmental data present
                r1 = scanner.fast_scan(G, verbose)

            for i in idx:
                i = _2d_sel(i)
                g = asarray(G[:, i], float) # for each genetic candidate (column in G), it extracts the data for that candidate and converts it to an array of floats.
