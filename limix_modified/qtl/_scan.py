import sys
from limix_modified._display._session import session_line, session_block
from limix_modified._data._asarray import asarray as _asarray
...
from .result._st_factory import STScanResultFactory
from .result._mt_factory import MTScanResultFactory 


def scan(
    G, Y, lik="normal", K=None, M=None, idx=None, A=None, A0=None, A1=None, rank=None, original_L_init=False, verbose=True
):
    """
    ...
    idx : list
        ...
        tests.
    A : General design matrix for trait-trait relationships.
        p×p array_like
        Symmetric trait-by-trait design matrix.
    A0 : Design matrix for the null hypothesis
        p×p₀ array_like, optional
        Matrix A₀, possibility a non-symmetric one. If ``None``, it defines an empty
        matrix, p₀=0. Defaults to ``None``.
    A1 : Design matrix for the alternative hypothesis.
        p×p₁ array_like, optional
    ...
    with session_block("QTL analysis", disable=not verbose):
        ...
        if A is None:
            ...
            r = _multi_trait_scan(idx, lik, Y, M, G, QS, A, A0, A1, rank, original_L_init, verbose)
...
def _print_input_info(idx, lik, Y, M, G, K):
    from limix_modified._display._draw import draw_list, draw_title
    from limix_modified._display._aligned import AlignedText
...
def _multi_trait_scan(idx, lik, Y, M, G, QS, A, A0, A1, rank, original_L_init, verbose):
    ...
    if lik[0] == "normal":
        scanner, C0, C1 = _mt_lmm(Y, A, M, QS, rank, original_L_init, verbose)
    else:
...
def _mt_lmm(Y, A, M, QS, rank, original_L_init, verbose):
    from limix_modified.glimix_core_mod.lmm._kron2sum import Kron2Sum
    ...
    lmm = Kron2Sum(Y.values, A, M.values, KG, rank, original_L_init, restricted=False)
    lmm.fit(verbose=verbose)
    sys.stdout.flush()
...
def _normalise_scan_names(r):
    from .result._tuples import VariantResult
