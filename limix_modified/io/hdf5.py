def read_limix(filepath):
    ...
    Y.name = "phenotype"
    # Fetch the genotype matrix from the HDF5 file
    G = fetch(filepath, "genotype/matrix")
    # Read the row and column headers for the genotype matrix
    rows = _read_attrs(filepath, "genotype/row_header")
    cols = _read_attrs(filepath, "genotype/col_header")
    # Convert the headers and rename the sample ID to "sample"
