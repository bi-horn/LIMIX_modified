...
def manhattan(data, colora="#5689AC", colorb="#21334F", highlight_color="#FFA500", threshold=1e-7, pts_kws=None, ax=None):
    """
    ...
    highlight_color : matplotlib color
        Color for points above the threshold.
    threshold : float
        p-value threshold for highlighting points.
    ...
    for i, c in enumerate(unique(data["order"])):
        ...
        above_threshold = y > threshold_log
        ax.plot(x[~above_threshold], y[~above_threshold], **pts_kws)
        pts_kws["color"] = highlight_color
        ax.plot(x[above_threshold], y[above_threshold], **pts_kws)

    ax.set_xlim(data["abs_pos"].min(), data["abs_pos"].max())
    if ax.get_ylim()[1] > threshold_log:
        ax.set_ylim(0, ax.get_ylim()[1])  # Keep the original upper limit if it exceeds the threshold
    else:
        ax.set_ylim(0, threshold_log + 1)  # Set the upper limit to threshold_log + 1 if it is lower
...
def _abs_pos(data): # changed beuace spacing too wide
