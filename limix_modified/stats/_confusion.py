...
import logging
#Jit: "Just-In-Time" compilation is a technique that converts Python code into machine code at runtime.
#This allows the code to run faster than it would in an interpreter.

#In summary, this code attempts to import the jit function from the numba module. If the import is successful, it returns the imported jit function.
#If the import fails, it defines a fallback jit function that simply returns its input.
def _get_jit():
    try:
        from numba import jit #The function jit is imported from the numba module.
    except ImportError: #If the import fails, a dummy function is defined and assigned to jit.

        def jit(x, *args, **kwargs): #The dummy function jit returns the input argument x.
            return x

    return jit

# these factory functions generate specialized versions of functions that find the indices of elements to the left and right of a given position in a list, based on a specified distance criterion
#The JIT compilation provided by the @jit decorator helps optimize the performance of these functions.
#The _walk_left function finds the index of the element to the left of a given position in a list, based on a specified distance criterion.
def _get_walk_left():
    jit = _get_jit()
...
#The getter function is a decorator that returns an object that can be used to access the elements of a function.
def getter(func):
...
class ConfusionMatrix(object):
    #P represents the number of positive instances, N represents the number of negative instances, true_set is an array of true labels, and idx_rank is an array of ranked indices.
    def __init__(self, P, N, true_set, idx_rank):
        from numpy import empty, asarray, searchsorted
        #The confusion matrix is initialized with empty arrays for true positives and false positives.
        ...
        self._N = N
        self._P = P
    # property is a built-in decorator in Python that allows you to define a method but access it like an attribute.
    @property
    def TP(self):
        return getter(lambda i: self._TP[i]) #The getter function is used to access the elements of the _TP array.
