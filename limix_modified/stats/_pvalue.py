...
def empirical_pvalues(xt, x0):
    ...
    #The function is defined with two parameters: xt and x0. These parameters represent the test statistics observed on the data (xt) and the null test statistics (x0)
    xt = asarray(xt, float)
    x0 = asarray(x0, float)
    
    #The idxt and idx0 variables are created by sorting the xt and x0 arrays in descending order using the argsort function from numpy. These variables store the indices that would sort the arrays in descending order.
    idxt = argsort(xt)[::-1]
    idx0 = argsort(x0)[::-1]
    xts = xt[idxt]
    x0s = x0[idx0]
