...
def normalize(data, spec):
    from _data._conf import CONF
    from qc._mean_std import mean_standardize
    from qc._quant_gauss import quantile_gaussianize
    ...
    if method == "gaussianize":
        x = quantile_gaussianize(x, axis=axis)
    elif method == "mean_std":
        x = mean_standardize(x, axis=axis)
    else:
