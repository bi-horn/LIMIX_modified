''' copied 1-to1 from https://github.com/limix/optimix/blob/master/optimix/_function.py and only adjusted __try_minimize to enable Python's stdout'''

import abc

from optimix._exception import OptimixError
...
class FuncOpt(metaclass=abc.ABCMeta):
    ...
    def _minimize(self, verbose=True, factr=FACTR, pgtol=PGTOL):
        ...
        if r is None or not isinstance(r, tuple) or len(r) < 3:
    ...
    def __try_minimize(self, n, factr, pgtol):
        ...
        # Python-side progress (Fortran disp bypasses Python stdout)
        self.__iter_count = getattr(self, '_FuncOpt__iter_count', 0)
        def _callback(x):
            self.__iter_count += 1
            if self.__verbose:
                val = self.__sign * self.value()
                grad = self.gradient()
                flat_g = np.concatenate([np.atleast_1d(grad[n]).ravel() for n in self.__varnames()])
                pg_norm = np.max(np.abs(flat_g))
                print(f"  Iter {self.__iter_count}: LML = {val:.6f}  |proj g| = {pg_norm:.5E}", flush=True)
        ...
        try:
            ...
            res = fmin_l_bfgs_b(
                self, x0, bounds=bounds, factr=factr, pgtol=pgtol,
                disp=0, callback=_callback if self.__verbose else None
            )
